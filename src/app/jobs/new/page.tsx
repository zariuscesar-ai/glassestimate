...
interface Inv { id: number; invoice_number: string; type: string; total: number; }
...
export default function NewJobPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/jobs', {
        ...
        body: JSON.stringify({
          client_id: parseInt(clientId), name, description,
          start_date: startDate, end_date: endDate, priority,
          assigned_crew: assignedCrew, job_site_address: jobAddress,
          notes, estimate_id: estimateId ? parseInt(estimateId) : null,
          total_linear_ft: totalLFt, total_sq_ft: totalSqFt,
          door_count: doorCount, glass_types: glassTypes,
        }),
      });
  ...
  if (loading) {
    return <div className="flex justify-center py-20"><p className="text-slate-400 text-lg">Loading...</p></div>;
  }
        ...
        <div className="card p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="input" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Office Renovation" required />
            </div>
          </div>
          <div>
            <label className="label">Description</label>
            <textarea className="input" rows={2} value={description} onChange={e => setDescription(e.target.value)} placeholder="Scope of work..." />
          </div>
          <div className="grid grid-cols-2 gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="label">Assigned Crew</label><input className="input" value={assignedCrew} onChange={e => setAssignedCrew(e.target.value)} placeholder="Team A" /></div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="label">Glass Types</label><input className="input" value={glassTypes} onChange={e => setGlassTypes(e.target.value)} placeholder='1/2" Tempered' /></div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="label">Link Estimate</label>
              <select className="select" value={estimateId} onChange={e => setEstimateId(e.target.value)}>
                <option value="">None</option>
                {estimates.map(e => <option key={e.id} value={e.id}>{e.invoice_number}</option>)}
              </select>
            </div>
          </div>
